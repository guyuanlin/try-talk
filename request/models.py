...
REQUEST_HEAD_FIELDS = getattr(settings, 'REQUEST_HEAD_FIELDS', [])
...
class Request(models.Model):
	...
	header = models.TextField(_('header'), null=True, blank=True)
	url_params = models.TextField(_('url params'), null=True, blank=True)
	post_params = models.TextField(_('post params'), null=True, blank=True)
	...
	class Meta:
		verbose_name = _(u'HTTP Requests 紀錄')
		verbose_name_plural = _(u'HTTP Requests 紀錄')
		ordering = ('-time',)
	...
	def from_http_request(self, request, response=None, commit=True):
		...
		self.url_params = str(request.GET)
		self.post_params = str(request.POST)

		self.header = {}
		for field in REQUEST_HEAD_FIELDS:
			if field in request.META:
				self.header[field] = request.META[field]

		# User infomation
		if 'HTTP_X_REAL_IP' in request.META:
			# for deploying behind Nginx
		...
		if getattr(request, 'user', False):
			# print str(request.user) + ": " + str(request.user.is_authenticated())
